...
class TriggerEngineTests(unittest.TestCase):
    ...
    def test_a_short_heartbeat_does_not_multiply_alarm_records(self):
        """The 2026-08-26 fault, pinned.

        A heartbeat firing while an alarm was latched used to be relabelled
        "Sustained Alarm" with is_heartbeat False, so it was stored and drawn
        as a fresh alarm record. Alarm traffic therefore scaled with the
        HEARTBEAT rate: at hb=1min a single latched High Temp produced a
        visible alarm record every 61 seconds instead of the six per half
        hour alarm_repeat_s asks for.

        Heartbeat rate is a liveness decision; alarm rate is a safety
        decision. Changing one must not silently change the other.
        """
        import amu_config
        amu_config.heartbeat_interval = 60          # the aggressive setting
        self.engine.alarm_repeat_s = 300.0

        # Latch a High Temp alarm, then hold it for half an hour.
        self.engine.evaluate(reading(temp=40), False)

        announced = 0
        heartbeats = 0
        for _ in range(180):                        # 30 minutes at 10 s steps
            self.clock.advance(10)
            trigger, cause, is_hb = self.engine.evaluate(reading(temp=40), False)
            if not trigger:
                continue
            if is_hb:
                heartbeats += 1
                self.assertEqual(cause, "Routine Heartbeat",
                                 "a heartbeat must not carry an alarm label")
            elif cause.startswith("Sustained Alarm"):
                announced += 1

        self.assertLessEqual(
            announced, 7,
            "a latched alarm must re-announce on alarm_repeat_s (~6 times in "
            "30 min), not on the heartbeat - got %d visible alarm records "
            "while the heartbeat was 60 s" % announced)
        self.assertGreater(
            heartbeats, 0,
            "the unit must still heartbeat while alarmed - that is how a "
            "quiet device learns the time, a config change or a revocation")
